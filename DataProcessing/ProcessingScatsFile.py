...
df = sqlContext.read.csv(volume_data_filepath, header=True)
...
i = 0; count = 0; initialCount = len(df.columns)-4
while i < initialCount:
    count = count + 1
    df = df.withColumn("sum_" + str(count),
                       df[df.columns[i+0]] + df[df.columns[i+1]] + df[df.columns[i+2]] + df[df.columns[i+3]])
    i = i+4
df.show()
...
finalDf.show()
# finalDf.write.csv(newCsvPath, header = True)
# df.rdd.map(lambda x: ",".join(map(str, x))).coalesce(1).saveAsTextFile(newCsvPath+"file.csv")
finalDf.coalesce(1).write.format("com.databricks.spark.csv").option("header", "true").save(newCsvPath)