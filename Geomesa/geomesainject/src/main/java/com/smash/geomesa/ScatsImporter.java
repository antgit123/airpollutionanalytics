...
public class ScatsImporter {
    ...
    private void run(ScatsImporterOptions options) throws IOException {
        ...
        GeomesaDataUtil.saveFeatureType(options, FeatureFactory.createFeatureType());
        try (SparkSession ss = SparkSession.builder().config(sparkConf).getOrCreate()) {
            ...
            HashMap<String, String> scatsPointShp =
                    readWktCsv(options.hdfsURL, options.inputVolumeCSV, "point");
...
//                "AND b.QT_INTERVAL_COUNT = c.DT_GENERAL";
            String sql = "SELECT b.* FROM volumeData b";
...
//        scatsData.persist(StorageLevel.MEMORY_AND_DISK_SER());
            ...
            JavaRDD<SimpleFeature> resultRDD = scatsData.javaRDD().mapPartitionsWithIndex((idx, iter) -> {
                ...
                SimpleFeatureBuilder featureBuilder = FeatureFactory.getFeatureBuilder();
                while (iter.hasNext()) {
                    ArrayList<SimpleFeature> simpleFeatures =
                            FeatureFactory.buildFeatureFromRow(iter.next(), featureBuilder);
...
//      geoMesaOpts.put("generateStats", Boolean.FALSE.toString());
            jsp.save(resultRDD, geoMesaOpts, FeatureFactory.FT_NAME);
        } // end of try block
    }
